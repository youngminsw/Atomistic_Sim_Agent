...
- [ ] 4. Implement workflow gate protocol and response validation
  What to do / Must NOT do: add `WorkflowGate` and gate response validation with Gajae-like fields and lifecycle semantics: pending, resolved, duplicate/idempotency handling, unknown/already-answered/replier-unavailable rejection. Gate and response records must carry stable owner/caller metadata, including `owner_agent_id`, `caller_agent_id`, `workflow_id`, `goal_id` when applicable, `gate_id`, status, and blocker codes. Add a shared `workflow_gate_response` runtime executor/tool contract (or equivalently named response surface) so all agents can answer gates through the same path. Enforce owner-scoped mutation in the runtime/executor: domain agents may mutate only their own workflows/goals, peer/equal-rank and domain-to-orchestrator mutations are denied, and `orchestrator` may mutate its own and subordinate domain-agent workflows/goals. Do not rely on prompts for authority enforcement, and do not couple this to a UI-only prompt path.
  Parallelization: Wave 2 | Blocked by: 2, 3 skeleton | Blocks: 5, 6, 7, 8, 9
  References (executor has NO interview context - be exhaustive): Gajae `test_protocol.py:200-217,304-342`; Gajae `test_client.py:872-891`; Gajae `actions.rs:52-237`; ASA `agent_runtime_tools.py:144-168`; ASA `tools.py:380-400,428-506`; ASA `agent_runtime/agent_registry.py:157-184`.
  Acceptance criteria (agent-executable): `python3 -m pytest -q tests/test_workflow_gate_protocol.py tests/test_workflow_goal_authority.py` proves flat gate payload shape, schema_hash stability, enum rejection, boolean approval acceptance, duplicate/idempotency behavior, unknown-gate rejection, shared `workflow_gate_response` executor output, explicit authority blockers such as `workflow_goal_authority_denied` or `workflow_goal_wrong_owner`, domain self-mutation success, peer/equal-rank denial, domain-to-orchestrator denial, and orchestrator-to-domain success.
  QA scenarios (name the exact tool + invocation): pytest plus direct Python smoke constructing a select gate and sending invalid/valid responses through `workflow_gate_response` with both same-owner and cross-owner caller/owner pairs; Evidence `.omo/evidence/task-4-asa-runtime-workflow-harness-parity.log`
...
- [ ] 7. Implement ultragoal runtime artifacts
  What to do / Must NOT do: implement ASA ultragoal-style deterministic durable goal state: `brief.md`, `goals.json`, `ledger.jsonl`, active-goal checkpoint, status summary, and blocked reconciliation when `codex_goal_snapshot` is missing or malformed. Goal records and checkpoint/ledger events must carry `owner_agent_id`, `created_by_agent_id`, mutation caller metadata, parent/orchestrator relationship metadata when relevant, and stable blocker/status codes for authority denials. Preserve append-only semantics when checkpointing; reject or block cross-owner mutation rather than silently rewriting another agent's goal. Do not invoke Codex goal-mode control APIs, OMO ultragoal, or hidden provider calls at runtime.
  Parallelization: Wave 3 | Blocked by: 3, 4 | Blocks: 8, 9, 10
  ...
  Acceptance criteria (agent-executable): `python3 -m pytest -q tests/test_ultragoal_runtime.py tests/test_workflow_goal_authority.py` proves initial goal files are created, each goal/checkpoint/ledger event records owner and caller metadata, ledger appends checkpoints, corrupt goal files block safely, duplicate checkpoints are handled deterministically, cross-owner checkpoint/mutation attempts are rejected or blocked, and missing Codex snapshot records `codex_snapshot_reconciled_or_blocked` as blocked not ready.
  QA scenarios (name the exact tool + invocation): Python test fixture with valid snapshot, missing snapshot, corrupt `goals.json`, duplicate checkpoint, same-owner checkpoint, peer-agent cross-owner checkpoint denial, and orchestrator-to-domain checkpoint/update success; Evidence `.omo/evidence/task-7-asa-runtime-workflow-harness-parity.log`
...
- [ ] 8. Wire workflow runtime through tool, TUI, catalog, and provider-visible schema
  What to do / Must NOT do: update `workflow_start` and `workflow_gate_response` executor/schema plus TUI slash handlers to expose gate metadata, artifact paths, blockers, response results, and ledger refs from the new runtime. Tool arguments/results must include owner/caller fields (`caller_agent_id`, `owner_agent_id`, `workflow_id`, `goal_id`, `gate_id`) and must route mutations through the authority-checked runtime from Todo 4. Preserve old commands and compatibility output where tests rely on it. Do not change unrelated tool definitions. Confirm every domain agent gets the same model-visible workflow tools through `COMMON_DOMAIN_TOOL_NAMES`, `DOMAIN_AGENT_TOOL_NAMES`, and `tool_registry_for_agent(...)` unless explicitly disabled by policy. Emit workflow loop runtime events and render active domain-agent workflow/goal loop state in the TUI activity rail and semantic output, including owner, workflow, goal, loop state, gate, checkpoint count, blocker, and ledger path.
  Parallelization: Wave 4 | Blocked by: 5, 6, 7 | Blocks: 10
  References (executor has NO interview context - be exhaustive): `sim_agent/agent_harness/tools.py:380-400,428-506`; `sim_agent/agent_harness/agent_runtime_tools.py:144-168`; `sim_agent/agent_runtime/live_agent_turn.py:151`; `sim_agent/cli/tui_workflow.py:16-77`; `sim_agent/cli/tui_catalog.py:32-38`; `sim_agent/cli/tui_agent_activity.py:26-41`; `sim_agent/cli/tui_activity_render.py:48-86`; `tests/test_agents_sdk_tool_gateway_runtime.py`; `tests/test_skill_workflow_runtime.py:61-100`; `tests/test_workflow_harnesses.py:44-75`.
  Acceptance criteria (agent-executable): `python3 -m pytest -q tests/test_workflow_harnesses.py tests/test_skill_workflow_runtime.py tests/test_agents_sdk_tool_gateway_runtime.py tests/test_workflow_tool_policy.py tests/test_tui_workflow_activity.py` passes with updated expectations for gate/artifact/response fields while preserving existing public command names, proving each supported agent sees both workflow tools through `tool_registry_for_agent(...)`, and proving activity rail plus semantic output render active workflow/goal loop fields.
  QA scenarios (name the exact tool + invocation): scripted TUI session with `/workflow deep-interview`, `/workflow-response <gate-id> <json-value>`, `/ralplan`, `/ultragoal`, plus a domain-agent workflow loop; provider payload schema inspection proving `workflow_start` and `workflow_gate_response` are model-visible to the shared agent tool registry; direct renderer test/event fixture proving activity rail and semantic lines include `owner_agent_id`, `workflow_id`, `goal_id`, `loop_state`, `gate_id`, checkpoint count, blocker, and ledger path; Evidence `.omo/evidence/task-8-asa-runtime-workflow-harness-parity.log`
