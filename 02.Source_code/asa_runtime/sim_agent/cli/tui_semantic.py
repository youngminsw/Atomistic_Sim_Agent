...
SEMANTIC_TTY_ENV = "ASA_TUI_SEMANTIC"
...
def filter_semantic_tty_output(output_stream: TextIO) -> TextIO:
    if output_stream.isatty() and not _semantic_tty_enabled():
        return SemanticFilteredTextIO(output_stream)
    return output_stream


def semantic_output_enabled(output_stream: TextIO) -> bool:
    return not output_stream.isatty() or _semantic_tty_enabled()
...
def _semantic_tty_enabled() -> bool:
    value = os.environ.get(SEMANTIC_TTY_ENV, "").strip().lower()
