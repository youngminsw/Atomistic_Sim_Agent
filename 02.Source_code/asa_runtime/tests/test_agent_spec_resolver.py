...
def test_subagent_presets_are_clean_room_non_persistent_and_bounded() -> None:
    ...
    expected_names = ("planner", "architect", "critic", "executor")
    ...
    assert "verifier" not in SUBAGENT_PRESETS
...
def test_subagent_presets_expose_bounded_report_tool_surface_without_bash() -> None:
    ...
    presets = tuple(resolve_subagent_preset(name) for name in ("planner", "architect", "critic", "executor"))
