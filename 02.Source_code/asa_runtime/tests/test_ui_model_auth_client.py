...
def test_model_auth_controller_posts_login_and_redacts_token() -> None:
    ...
    script = "\n".join(
        [
            ...
            "nodes['model-provider'].value = 'openai-codex';",
            ...
            "  if (url === '/api/model/auth/status') {",
            "    return Promise.resolve({ ok: true, json: () => Promise.resolve({ ok: true, providers: [], connected_provider_count: 0 }) });",
            "  }",
            "  return Promise.resolve({ ok: true, json: () => Promise.resolve({ ok: true, provider: 'openai-codex', logged_in: true }) });",
            "};",
            "async function main() {",
            "  auth.mount(documentRef, fetcher);",
            "  await new Promise((resolve) => setImmediate(resolve));",
            "  if (calls[0].url !== '/api/model/auth/status') throw new Error('mount did not refresh auth status');",
            "  await nodes['model-auth-form'].trigger('submit');",
            "  const loginCall = calls.find((call) => call.url === '/api/model/auth/login');",
            "  if (!loginCall) throw new Error('bad auth endpoint');",
            "  if (!loginCall.body.includes('ui-secret-token')) throw new Error('token not submitted');",
            "  if (!nodes['model-auth-status'].textContent.includes('openai-codex logged in')) throw new Error('status not rendered');",
