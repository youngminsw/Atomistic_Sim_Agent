...
This directory is the new production runtime boundary for the Atomistic Simulation
Agent (ASA). Keep new runtime code here, not under `../mss_agent`.

## Source split

- `02.Source_code/asa_runtime` is the new runtime package:
  ...
  - Runtime tests: `tests`
- `02.Source_code/mss_agent` is legacy/prototype reference code. Do not put new
  runtime code there.
- It is acceptable for ASA runtime to reference selected legacy assets, especially
  force-field and fixture files under `../mss_agent/md_agent_window/Reference` and
  `../mss_agent/md_agent_window/results`.
